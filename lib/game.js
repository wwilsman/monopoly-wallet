...
export default class MonopolyGame {
  ...
  join({ name = '', token = '' }) {
    ...
    assert(`${name} is already playing`, this.state.players.find((p) => p._id === player._id))
    ...
    this.store.dispatch(actions.bankTransfer(player._id, -this.state.start))
  ...
  claimBankrupty(playerID, beneficiaryID) {
    let player = this.getPlayer(playerID)
