...
export default class MonopolyGame {
  ...
  join(name = '', token) {
    ...
    this.update(`{p:${token}} joined the game`, {
      players: [token]
    })
  }
  ...
  payBank(token, amount) {
    ...
    this.update(`{p:${token}} paid the bank {$:${amount}}`, {
      players: [token]
    })
  }
  ...
  collectMoney(token, amount) {
    ...
    this.update(`{p:${token}} collected {$:${amount}}`, {
      players: [token]
    })
  }
  ...
  payPlayer(fromToken, toToken, amount) {
    ...
    this.update(`{p:${fromToken}} paid {p:${toToken}} {$:${amount}}`, {
  ...
  payRent(token, propertyName, roll = 2) {
    ...
    this.update(`{p:${token}} paid {$:${rent}} in rent for ${propertyName}`, {
      players: [token, owner]
    })
  }
  ...
  claimBankruptcy(token, beneficiaryToken) {
    ...
    this.update(`{p:${beneficiaryToken}} bankrupt {p:${token}}`, {
  ...
  buyProperty(token, propertyName, price) {
    ...
    this.update(`{p:${token}} purchased ${propertyName}`, {
      players: [token]
    })
  }
  ...
  improveProperty(token, propertyName) {
    ...
    this.update(`{p:${token}} improved ${propertyName}`, {
      players: [token]
    })
  }
  ...
  unimproveProperty(token, propertyName, doMin) {
    ...
    this.update(`{p:${token}} unimproved ${propertyName}`, {
      players: [token]
    })
  }
  ...
  mortgageProperty(token, propertyName) {
    ...
    this.update(`{p:${token}} mortaged ${propertyName}`, {
      players: [token]
    })
  }
  ...
  unmortgageProperty(token, propertyName, noInterest) {
    ...
    this.update(`{p:${token}} unmortaged ${propertyName}`, {
      players: [token]
    })
  }
  ...
  auctionProperty(token, propertyName, participants) {
    ...
    this.update(`${propertyName} is up for auction.`, {
      players: [token],
  ...
  placeAuctionBid(token, propertyName, amount) {
    ...
    this.update(`{p:${token}} bid {$:${amount}} on ${propertyName}`, {
      players: [token],
      property: propertyName,
      action: 'auction:bid',
      save: false
  ...
  concedeAuction(token, propertyName) {
    ...
    this.update(`{p:${token}} conceded from the auction`, {
      players: [token],
      property: propertyName,
  ...
  _closeAuction() {
    ...
    if (winning && winning.amount > 0) {
      ...
      this.update(`{p:${winning.player}} bought ${auction.property} for {$:${winning.amount}}`, {
      ...
      this.update(`${auction.property} auction cancelled`, {
        action: 'auction:end',
        save: false
      })
    }
  ...
  makeTrade(token, withToken, trade) {
    ...
    this.update(`{p:${token}} made an offer with {p:${withToken}}`, {
      players: [token, withToken],
      action: 'trade:new',
      save: false
    })
  ...
  cancelTrade(token, withToken) {
    ...
    this.update(`{p:${token}} declined to trade with {p:${withToken}}`, {
      players: [token, withToken],
      action: 'trade:cancel',
      save: false
    })
  ...
  finalizeTrade(token, withToken) {
    ...
    this.update(`{p:${token} traded with {p:${withToken}}`, {
      players: [token, withToken],
      action: 'trade:done'
    })
  }
