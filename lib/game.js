...
export default class MonopolyGame {
  ...
  constructor(state) {
    ...
    this.state = { ...state,
      ...
      players: state.players.map((player) => {
        const _id = dasherize(`${player.name}-${player.token}`)
        return { _id, ...playerDefaults, ...player }
      }),

      properties: state.properties.map((property) => {
        const _id = dasherize(property.name)
        const mortgage = Math.round(property.price * state.mortgageRate)
        const interest = Math.round(mortgage * state.interestRate)
        return { _id, ...propertyDefaults, ...property, mortgage, interest }
      })
    }
  ...
  mortgageProperty(playerID, propertyID, unimprove) {
    ...
    this.store.dispatch(actions.bankTransfer(player._id, -property.mortgage))
