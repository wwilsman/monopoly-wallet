import { assert, dasherize } from './helpers'
...
export default class MonopolyGame {
  ...
  constructor(state) {
    ...
    this.state = { ...state,
      ...
      players: state.players.map(playerDefaults),
      properties: state.properties.map(propertyDefaults)
    }
    ...
    this.update = (message) => {
      this.store.dispatch(actions.saveState(message))
  ...
  join(name = '', token = '') {
    ...
    this.update(`${name} has joined the game`)
  ...
  payBank(playerID, amount) {
    ...
    this.update(`${player.name} paid the bank $${amount}`)
  }
  ...
  collectMoney(playerID, amount) {
    ...
    this.update(`${player.name} collected $${amount}`)
  }
  ...
  buyProperty(playerID, propertyID, price) {
    ...
    this.update(`${player.name} purchased ${property.name}`)
  }
  ...
  improveProperty(playerID, propertyID) {
    ...
    this.update(`${player.name} improved ${property.name}`)
  }
  ...
  unimproveProperty(playerID, propertyID, doMin) {
    ...
    this.update(`${player.name} unimproved ${property.name}`)
  }
  ...
  mortgageProperty(playerID, propertyID, unimprove) {
    ...
    this.update(`${player.name} mortaged ${property.name}`)
  }
  ...
  unmortgageProperty(playerID, propertyID, noInterest) {
    ...
    this.update(`${player.name} unmortaged ${property.name}`)
  }

  makeTransaction(transaction1, transaction2) {
    ...
    this.update(`${player1.name} made a transaction with ${player2.name}`)
  }
  ...
  claimBankruptcy(playerID, beneficiaryID) {
    ...
    this.update(`${player.name} claimed bankruptcy`)
  }

  undo(entryID) {
    let { past, present } = this.store.getState()
    let index = past.reduce((t, e, i) => e.entry === entryID ? i - 1 : t, -1)

    if (index < 0 && present.entry === entryID) {
      index = past.length - 1
    }

    assert('Cannot find entry in history', index < 0)

    this.store.dispatch(undoActions.jumpToPast(index))

    this.update(`Game reset to when ${past[index].note}`)
  }
