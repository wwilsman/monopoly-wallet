...
export default class MonopolyGame {
  ...
  join(name = '', token) {
    ...
    this.update('player', `{p:${token}} joined the game`, [token])
  }
   ...
   * @param {string} propertyName
   */
  auctionProperty(token, propertyName) {
    const property = this.getProperty(propertyName)

    assert('There is already an active auction', this.state.auction)
    assert('Cannot auction player properties', property.owner !== 'bank')

    this.store.dispatch(actions.startAuction(propertyName))

    this.update('auction', `${propertyName} is up for auction.`, [token])
  }
  ...
  placeAuctionBid(token, propertyName, amount) {
    ...
    this.update('auction', `{p:${token}} bid {$:${amount}}`, [token])
  }

  /**
   * Concede from an auction and maybe announce a winner
   *
   * @param {string} token
   * @param {string} propertyName
   */
  concedeAuction(token, propertyName) {
    ...
    this.update('auction', `{p:${token}} conceded from the auction`, [token])

    if (this.state.auction.bids.length === 1) {
      const winning = this.state.auction.bids[0]

      this.store.dispatch(actions.endAuction())
      this.store.dispatch(actions.bankTransfer(winning.player, winning.amount))
      this.store.dispatch(actions.transferProperty(winning.player, propertyName))

      this.update(
        'game',
        `{p:${winning.player}} bought ${propertyName} for {$:${winning.amount}}`,
        [winning.player]
      )
    }
  }
