...
export default class MonopolyGame {
  ...
  constructor(config, state = { properties: [] }) {
    ...
    this.update = (message, meta = {}) => {
      this.store.dispatch(actions.saveState(message, meta))
  ...
  join(name = '', token) {
    ...
    this.update(`{t:${token}} joined the game`, {
      players: [token]
    })
  }
  ...
  undo(entryID) {
    ...
    this.update(`Game reset to before ${past[index].notice.message}`)
  }
  ...
  payBank(token, amount) {
    ...
    this.update(`{t:${token}} paid the bank {$:${amount}}`, {
      players: [token]
    })
  }
  ...
  collectMoney(token, amount) {
    ...
    this.update(`{t:${token}} collected {$:${amount}}`, {
      players: [token]
    })
  }
  ...
  payPlayer(fromToken, toToken, amount) {
    ...
    this.update(`{t:${fromToken}} paid {t:${toToken}} {$:${amount}}`, {
      players: [fromToken, toToken]
    })
  }
  ...
  payRent(token, propertyName, roll = 2) {
    ...
    assert(`{p:${propertyName}} is unowned`, property.owner === 'bank')
    assert(`{p:${propertyName}} has been mortgaged`, property.isMortgaged)
    ...
    this.update(`{t:${token}} paid {$:${rent}} in rent for {p:${propertyName}}`, {
      players: [token, owner]
    })
  }
  ...
  claimBankruptcy(token, beneficiaryToken) {
    ...
    this.update(`{t:${beneficiaryToken}} bankrupt {t:${token}}`, {
      players: [token, beneficiaryToken]
    })
  }
  ...
  buyProperty(token, propertyName, price) {
    ...
    assert(`You already own {p:${propertyName}}`, property.owner === token)
    assert(`{p:${propertyName}} is already owned`, property.owner !== 'bank')
    assert(`{p:${propertyName}} is being auctioned`, auction && auction.property === propertyName)
    ...
    this.update(`{t:${token}} purchased {p:${propertyName}}`, {
      players: [token]
    })
  }
  ...
  improveProperty(token, propertyName) {
    ...
    assert(`You do not own {p:${propertyName}}`, property.owner !== token)
    ...
    assert(`{p:${propertyName}} is already fully improved`, property.buildings === 5)
    ...
    this.update(`{t:${token}} improved {p:${propertyName}}`, {
      players: [token]
    })
  }
  ...
  unimproveProperty(token, propertyName, doMin) {
    ...
    assert(`{p:${propertyName}} is already fully unimproved`, property.buildings === 0)
    assert('Must build evenly', group.some((p) => p.buildings > property.buildings))
    ...
    this.update(`{t:${token}} unimproved {p:${propertyName}}`, {
      players: [token]
    })
  }
  ...
  mortgageProperty(token, propertyName) {
    ...
    assert(`You do not own {p:${propertyName}}`, property.owner !== token)
    assert(`{p:${propertyName}} is already mortgaged`, property.isMortgaged)
    assert(`{p:${propertyName}} still has improvements`, property.buildings > 0)
    ...
    this.update(`{t:${token}} mortaged {p:${propertyName}}`, {
      players: [token]
    })
  }
  ...
  unmortgageProperty(token, propertyName, noInterest) {
    ...
    assert(`You do not own {p:${propertyName}}`, property.owner !== token)
    assert(`{p:${propertyName}} is not mortgaged`, !property.isMortgaged)
    ...
    this.update(`{t:${token}} unmortaged {p:${propertyName}}`, {
      players: [token]
    })
  }
  ...
  auctionProperty(token, propertyName, participants) {
    ...
    this.update(`{p:${propertyName}} is up for auction.`, {
      players: [token],
  ...
  placeAuctionBid(token, propertyName, amount) {
    ...
    assert(`{p:${propertyName}} is not up for auction`, !auction || auction.property !== propertyName)
    ...
    assert(`You need to bid higher than {t:${auction.winning}}`, winning.amount >= amount)
    assert('Insufficient funds', player.balance < amount)

    this.store.dispatch(actions.placeAuctionBid(token, amount))

    this.update(`{t:${token}} bid {$:${amount}} on {p:${propertyName}}`, {
      players: [token],
      property: propertyName,
      action: 'auction:bid',
      save: false
  ...
  concedeAuction(token, propertyName) {
    ...
    // more than one player or one player that hasn't bid
    if (bids.length > 1 || (bids.length === 1 && bids[0].amount === 0)) {
      this.update(`{t:${token}} conceded from the auction`, {
        players: [token],
        property: propertyName,
        action: 'auction:concede',
        save: false
      })

    // last to concede or one player left with bid
    } else if (bids.length < 1 || (bids.length === 1 && bids[0].amount > 0)) {
      this._closeAuction()
    }
  }
  ...
  _closeAuction() {
    ...
    if (winning && winning.amount > 0) {
      ...
      this.update(`{t:${winning.player}} won {p:${auction.property}} for {$:${winning.amount}}`, {
        players: [winning.player],
        property: auction.property,
        action: 'auction:end'
      })

    } else if (auction) {
      this.update(`{p:${auction.property}} auction cancelled`, {
  ...
  _validateTrade(trade) {
    ...
    assert(`{t:${player2.token}} has insufficient funds`, player2.balance - amount2 - interest1 < 0)
    assert(`You do not own {p:${unowned1 && unowned1.name}}`, unowned1)
    assert(`{p:${unowned2 && unowned2.name}} not owned by {t:${player2.token}}`, unowned2)
    assert(`{p:${improved && improved.name}} has improvements`, improved)
  ...
  makeTrade(token, withToken, trade) {
    ...
    this.update(`{t:${token}} made an offer with {t:${withToken}}`, {
      players: [token, withToken],
      action: 'trade:new',
      save: false
    })
  ...
  cancelTrade(token, withToken) {
    ...
    assert('Cannot find trade with {t:${withToken}}', tradeIndex === -1)

    this.store.dispatch(actions.deleteTrade(tradeIndex))

    this.update(`{t:${token}} declined to trade with {t:${withToken}}`, {
      players: [token, withToken],
      action: 'trade:cancel',
      save: false
    })
  ...
  finalizeTrade(token, withToken) {
    ...
    assert('Cannot find trade with {t:${withToken}}', !trade)
    ...
    this.update(`{t:${token}} traded with {t:${withToken}}`, {
      players: [token, withToken],
      action: 'trade:done'
    })
  }
