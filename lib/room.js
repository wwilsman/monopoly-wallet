...
export default class MonopolyRoom {
  ...
  join(socket, name, token) {
    ...
    // player previously joined
      ...
      if (this.players.has(token)) {
        socket.emit('game:error', {
          message: `{t:${token}} is already playing`
        })
      } else {
        this.add(token, socket)
      }
      ...
      this.pollPlayers(`${name.toUpperCase()} would like to join`, (res) => {
        if (!res) {
          socket.emit('room:error', {
            name: 'Sorry',
            message: 'Your friends hate you'
          })
        } else {
          this.game.join(name, token)
          this.add(token, socket)
  ...
  add(token, socket) {
    ...
    socket.on('disconnect', () => {
      ...
      // force conceding in auctions
      if (this.game.state.auction) {
        const { property, bids } = this.game.state.auction

        if (bids.find((b) => b.player === token)) {
    ...
    const events = {
      ...
      'auction:start': ({ property }) =>
        this.game.auctionProperty(token, property, this.getActivePlayers()),
  ...
  contestGame(token, entryID) {
    ...
    this.pollPlayers(`{t:${token}} is contesting when ${message}`, (result) => {
      if (!result) {
        socket.emit('game:error', {
