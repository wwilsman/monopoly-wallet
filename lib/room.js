...
export default class MonopolyRoom {
  ...
  addPlayer(pid, socket) {
    ...
    // trading
    wrapped('make trade', loaded(this.makeOffer, this))
    wrapped('accept offer', loaded(this.acceptOffer, this))
    wrapped('decline offer', loaded(this.declineOffer, this))
  ...
  makeOffer(pid, otherPlayer, offer, trade) {
    ...
    socket.emit('trade offer', tradeID, pid, offer, trade)
  }

  // Accept a trade offer made by another player
  acceptOffer(pid, tradeID) {
    let tradeOffer = this.trades.get(tradeID)
    let player = this.game.getPlayer(tradeOffer.player === pid ? tradeOffer.with : tradeOffer.player)
  ...
  declineOffer(pid, tradeID) {
    ...
    assert(`${otherPlayer.name} didn't make you an offer`, tradeOffer.with !== pid && tradeOffer.player !== pid)

    if (tradeOffer.with === pid) {
      let socket = this.players.get(tradeOffer.player)
      socket.emit('decline trade', tradeID, `${player.name} has declined your offer`)
    }
