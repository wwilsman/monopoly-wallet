Monopoly.PlayerClass = function (game) {
  ...
  function Player(props) {
    ...
    Monopoly.Assert('Player cannot be named "bank"',
      game.Bank && (props.name === 'bank' || props.token === "bank"));
  ...
  Player.get = function(name) {
    ...
    Monopoly.Assert('Unknown player "%s"', !player, name);
  ...
  Player.prototype = {
    ...
    buy: function(propName, price) {
      var property = game.Property.get(propName),
    ...
    transfer: function(player, what) {
      ...
      default:
        ...
        if (typeof what === 'number') {
          ...
          Monopoly.Assert('LowBalanceError', what >= this.balance);
        ...
        // Transfer an asset
          ...
          Monopoly.Assert('OwnerError', !this.assets[what], 'any ' + what);
    ...
    improve: function(property) {
      ...
      Monopoly.Assert('OwnerError', this !== property.owner, property.name);

      // Player doesn not have enough currency
      Monopoly.Assert('LowBalanceError', property.costs.build >= this.balance);
    ...
    unmortgage: function(property) {
      ...
      // Player doesn't have enough currency
      Monopoly.Assert('LowBalanceError', property.costs.mortgage >= this.balance);
    ...
    // Get any properties
    get properties() {
      return game.Property.belongsTo(this);
    }
  };

  // Create the default "bank" player
  game.Bank = new Player({
    name: 'bank',
    token: 'bank',
