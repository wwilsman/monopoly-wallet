  ...
  switch (action.type) {
    ...
    case 'BANK_TRANSFER':
      return { ...state,
        ...
        players: state.players.map((p) => p.token !== action.player ? p :
    ...
    case 'TRANSFER_MONEY':
      return { ...state,
        players: state.players.map((p) =>
          p.token === action.fromPlayer ? { ...p, balance: p.balance - action.amount } :
          p.token === action.toPlayer ? { ...p, balance: p.balance + action.amount } : p
        )
      }
    case 'TRANSFER_PROPERTY':
      return { ...state,
        properties: state.properties.map((p) => p.name !== action.property ? p :
