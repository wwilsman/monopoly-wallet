...
module.exports = function (M) {
  ...
  function Player(props) {
    ...
    var isBank = props.name === 'bank' || props.token === "bank";
    MonopolyAssert('Player cannot be named "bank"', M.Bank && isBank);
  ...
  Player.get = function(name) {
    var player = Player.collection.filter(function(p) {
      return p.name === name;
    })[0];
  ...
  Player.prototype = {
    ...
    transfer: function(player, what) {
      ...
      default:
        ...
        // Transfer a property
        } else if (what instanceof M.Property) {
    ...
    unmortgage: function(property) {
      var cost = property.values.mortgage + property.costs.interest;

      // Player is not **property** owner
      MonopolyAssert('OwnerError', this !== property.owner, property.name);

      // Player doesn't have enough currency
      MonopolyAssert('LowBalanceError', cost >= this.balance);
    ...
    bankrupt: function(player) {
      ...
      // Sell all property improvements
