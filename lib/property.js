Monopoly.Property = (function () {
  ...
  function Property(props) {
    ...
    this.costs.mortgage = Math.round(this.costs.price * config.rates.mortgage);

    // Set this property's values
    this.values = {
      mortgage: Math.round(this.costs.price * config.rates.unmortgage),
      building: Math.round(this.costs.build * config.rates.building)
    };
  ...
  Property.prototype = {
    ...
    improve: function() {
      ...
      if (needsHotel) {
        ...
        // Not enough hotels
        Monopoly.Assert('AvailabilityError', config.availableHotels < 1, 'hotels');
        
        // Adjust available buildings
        config.availableHotels -= 1;
        config.availableHouses += 4;

      // Property needs a house
      } else {
        
        // Not enough houses
        Monopoly.Assert('AvailabilityError', config.availableHouses < 1, 'houses');
    ...
    unimprove: function() {
      ...
      if (this.isFullyImproved) {
        ...
        // Not enough houses
        Monopoly.Assert('AvailabilityError', config.availableHouses < 4, 'houses');
