Monopoly.PropertyClass = function (game) {
  ...
  function Property(props) {
    ...
    this.costs.mortgage = Math.round(this.costs.price * game.rates.mortgage);

    // Set this property's values
    this.values = {
      mortgage: Math.round(this.costs.price * game.rates.unmortgage),
      building: Math.round(this.costs.build * game.rates.building)
    };
  ...
  Property.get = function(name) {
    ...
    Monopoly.Assert('Unknown property "%s"', !property, name);
  ...
  Property.prototype = {
    ...
    improve: function() {
      ...
      Monopoly.Assert('MonopolyError', !this.isMonopoly, this.name);

      // Already fully improved
      Monopoly.Assert('FullImprovementError', this.isFullyImproved, this.name);

      // Must build evenly
      Monopoly.Assert('BuildEvenlyError', !this.isImprovedEvenly, this.name);

      // Property needs a hotel
      if (needsHotel) {

        // Not enough hotels
        Monopoly.Assert('AvailabilityError', game.availableHotels < 1, 'hotels');
      ...
      // Property needs a house
        ...
        // Not enough houses
        Monopoly.Assert('AvailabilityError', game.availableHouses < 1, 'houses');
    ...
    unimprove: function() {
      ...
      Monopoly.Assert('UnimprovementError', !this.isImproved, this.name);

      // Must build evenly
      Monopoly.Assert('BuildEvenlyError', !this.isUnimprovedEvenly);

      // Property has hotel
      if (this.isFullyImproved) {

        // Not enough houses
        Monopoly.Assert('AvailabilityError', game.availableHouses < 4, 'houses');
    ...
    transfer: function(player) {
      ...
      Monopoly.Assert('ImprovementError', this.anyImproved, this.name);
    ...
    mortgage: function() {
      ...
      Monopoly.Assert('MortgageError', this.isMortgaged, this.name);
    ...
    unmortgage: function() {
      ...
      Monopoly.Assert('UnmortgageError', !this.isMortgaged, this.name);
    ...
    // Property's owner
    get owner() {
      return game.Player.get(this.ownerName);
    },
